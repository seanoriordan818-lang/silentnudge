  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <button className="relative p-2 rounded-lg border border-border/50 hover:border-primary/30 transition-colors">
            ...
            <>
                ...
                <div className="space-y-4">
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-2">
                      <div className="w-16 h-16 bg-raised rounded-md overflow-hidden flex-shrink-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-semibold text-gold">${parseFloat(item.price.amount).toFixed(2)}</p>
                      </div>
                      <div className="flex flex-col items-end gap-2 flex-shrink-0">
                        <button className="p-1 hover:text-destructive transition-colors" onClick={() => removeItem(item.variantId)}>
                          <Trash2 className="h-3.5 w-3.5" />
                        </button>
                        <div className="flex items-center gap-1">
                          <button className="w-6 h-6 rounded border border-border flex items-center justify-center hover:border-primary/30" onClick={() => updateQuantity(item.variantId, item.quantity - 1)}>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t border-border">
                ...
                <button
                  ...
                  className="w-full py-3.5 rounded-full bg-primary text-primary-foreground font-bold text-sm flex items-center justify-center gap-2 shadow-gold disabled:opacity-50"
