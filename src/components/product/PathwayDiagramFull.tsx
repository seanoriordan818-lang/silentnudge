const ArrowDown = ({ color }: { color: 'stress' | 'calm' }) => (
  <div className="flex justify-center items-center h-[30px] w-full">
    <svg width="12" height="22" viewBox="0 0 12 24" fill="none" stroke="currentColor" strokeWidth="2"
...
const StepCard = ({ icon, title, desc, type }: { icon: string; title: string; desc: string; type: 'stress' | 'calm' }) => (
  <div className="w-full max-w-[360px] rounded-[14px] py-[18px] px-5"
    style={{
    ...
    <div className="flex items-start gap-2.5">
      ...
      <div className="min-w-0">
        <div className="font-display text-[17px] font-bold leading-tight mb-1"
          style={{ color: type === 'stress' ? '#8B2020' : 'hsl(var(--gold))' }}>{title}</div>
        <div className="text-[13px] leading-snug" style={{ color: 'rgba(255,255,255,0.45)' }}>{desc}</div>
      </div>
    </div>
  </div>
);

const ResultCard = ({ icon, title, lines, type }: { icon: string; title: string; lines: string[]; type: 'stress' | 'calm' }) => (
  <div className="w-full max-w-[360px] rounded-[14px] p-5 text-center"
    style={{
      background: type === 'stress'
        ? 'linear-gradient(135deg, #1e1520 0%, #1a1018 100%)'
        : 'linear-gradient(135deg, #1e1a10 0%, #181510 100%)',
      border: `1px solid ${type === 'stress' ? 'rgba(139,32,32,0.4)' : 'rgba(212,146,10,0.4)'}`,
      boxShadow: type === 'stress' ? '0 0 30px rgba(139,32,32,0.1)' : '0 0 30px rgba(212,146,10,0.12)',
    }}>
    <span className="text-[26px] block">{icon}</span>
    <div className="font-display text-[19px] font-bold mb-1"
      style={{ color: type === 'stress' ? '#8B2020' : 'hsl(var(--gold))' }}>{title}</div>
    {lines.map((l, i) => (
...
export const PathwayDiagramFull = () => (
  <div className="w-full rounded-[20px] py-8 md:py-11 px-5 md:px-9 flex flex-col overflow-hidden"
    style={{ background: 'linear-gradient(180deg, #141220 0%, #0f0d18 100%)' }}>

    <div className="flex flex-row flex-nowrap items-stretch w-full">
      {/* LEFT — Stress */}
      <div className="flex-1 min-w-0 flex flex-col items-center">
        <div className="text-[11px] font-semibold tracking-[3px] uppercase text-center w-full mb-6 pb-2.5"
        ...
        <ArrowDown color="stress" />
        <ResultCard icon="❌" title="Panic Wake-Up" lines={['Groggy. Stressed. Irritable.', 'Your partner wakes up too.']} type="stress" />
      </div>

      {/* DIVIDER */}
      <div className="flex-[0_0_60px] w-[60px] flex flex-col items-center justify-center">
        ...
        <div className="w-11 h-11 rounded-full flex items-center justify-center font-display text-sm font-bold my-2 shrink-0"
      ...
      <div className="flex-1 min-w-0 flex flex-col items-center">
        ...
        <ArrowDown color="calm" />
        <ResultCard icon="✅" title="Gentle Wake-Up" lines={['Alert. Calm. No cortisol spike.', 'Your partner feels nothing.']} type="calm" />
      </div>
    </div>

    <p className="text-center mt-7 text-[12px] italic tracking-wide" style={{ color: 'rgba(255,255,255,0.2)' }}>
