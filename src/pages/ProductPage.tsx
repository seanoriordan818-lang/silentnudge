...
import { Check, Lock, Truck, RefreshCw, Loader2 } from 'lucide-react';
...
import { ColorSelector, ColorAllocation } from '@/components/product/ColorSelector';
...
const ProductPage = () => {
  ...
  const addBundleItems = useCartStore((state) => state.addBundleItems);
  const isCartLoading = useCartStore((state) => state.isLoading);
  const openDrawer = useCartStore((state) => state.openDrawer);

  // Reset color counts when bundle changes
  useEffect(() => {
    if (selectedBundle === 'couples') {
      setColorCounts({ Black: 1, White: 1 });
    } else if (selectedBundle === 'family') {
      setColorCounts({ Black: 2, White: 2 });
    }
  }, [selectedBundle]);
  ...
  const colors = product?.node?.options?.find(o => o.name === 'Color')?.values || ['Black', 'White'];

  const findVariant = (color: string) => variants.find(v => v.node.title === color || v.node.selectedOptions?.some(o => o.name === 'Color' && o.value === color));

  const currentBundle = bundles.find((b) => b.id === selectedBundle) || bundles[0];
  const total = currentBundle.price;
  const totalRequired = BUNDLE_TOTALS[selectedBundle] || 1;
  const currentColorTotal = Object.values(colorCounts).reduce((a, b) => a + b, 0);
  const isBundleValid = selectedBundle === 'single' || currentColorTotal === totalRequired;

  const handleUpdateColorCount = (color: string, delta: number) => {
    setColorCounts(prev => {
      const newVal = Math.max(0, (prev[color] || 0) + delta);
      const otherTotal = Object.entries(prev)
        .filter(([k]) => k !== color)
        .reduce((sum, [, v]) => sum + v, 0);
      if (newVal + otherTotal > totalRequired) return prev;
  ...
  const handleAddToCart = async () => {
    ...
    if (selectedBundle === 'single') {
      const variant = findVariant(selectedColor);
      if (!variant) return;
      await addItem({
        product,
      ...
      // Bundle: add multiple variant line items
      if (!isBundleValid) return;
      const discountCode = BUNDLE_DISCOUNT_CODES[selectedBundle];
      const bundleItems = colors
        .filter(color => (colorCounts[color] || 0) > 0)
        .map(color => {
      ...
      await addBundleItems(bundleItems, discountCode, selectedBundle);
    }
    openDrawer();
  };
  ...
  return (
    <>
      {/* ─── HERO: Image + Details ─── */}
      <section className="max-w-[1100px] mx-auto px-5 md:px-7 pt-4 pb-2">
        <div className="flex flex-col md:flex-row md:gap-10 md:items-start">
          {/* ─── LEFT: Image Gallery ─── */}
          <div className="md:w-[55%] md:sticky md:top-24">
          ...
          <div className="md:w-[45%] pt-5 md:pt-0">
            <Reveal>
              {/* Rating */}
              ...
              {/* Product name */}
              <p ref={nameRef} className="font-display text-[22px] md:text-[26px] font-semibold leading-[1.2] mb-2 text-muted-foreground">
              ...
              <div className="flex items-baseline gap-3 mb-3">
                ...
                <span className="font-display text-[24px] md:text-[26px] text-gold">${currentBundle.price}</span>
              ...
              <div className="mb-5">
                {selectedBundle === 'single' ? (
                  ...
                  <ColorAllocation
                    ...
                    onUpdateCount={handleUpdateColorCount}
